...
def make_volcano_plot(df_, method, output_plot_path, modality, top_k_to_label=1):
    ...
    if modality == 'radiology':
        ...
        if sig_threshold != -1:
            ...
            for index, row in df[:top_k_to_label].iterrows():
                ...
                if x > 0:
                    ...
                    align = 'left'
                    x_offset = 0.1
                ax.text(x+x_offset, y-0.1, feat, horizontalalignment=align, size=6, color='black')
...
if __name__ == '__main__':
    ...
    PARSER.add_argument('--train_mrn_df_path', type=str, help='path to pd df with train mrns, optional', default=None)
    ...
    PARSER.add_argument('--index_col', type=str, default='mrn', help="name of column to set as index")
    ...
    if ARGS.modality == 'radiology':
        ...
        DF = DF.filter(regex='wavelet|log-1|log-3')
    if ARGS.outcome_df_path:
        OUTCOME_DF = pd.read_csv(ARGS.outcome_df_path).set_index(ARGS.index_col)[['duration', 'observed']]
        DF = DF.join(OUTCOME_DF, how='inner')
    if ARGS.train_mrn_df_path:
        DF = DF[DF.index.isin(pd.read_csv(ARGS.train_mrn_df_path).mrn)]

    if ARGS.bootstrap:
        DF = DF.sample(frac=0.95)

    # genom_train = pd.read_csv('/Users/boehmk/shahLab/thesis/integrated-analysis/_genom_train_df.csv').rename(columns={'mrn': 'ID'})
    # genom_train.ID = genom_train.ID.astype(int)
    # genom_train = genom_train.set_index("ID")
    # DF = DF.join(genom_train, how='inner')
